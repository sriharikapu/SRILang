...
docs:
	rm -f docs/srilang.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ -d 2 srilang/
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
	open docs/_build/html/index.html

docker-build:
	@docker build -t srilang \
...
docker-release:
	docker login
	docker tag srilang srilanglang/srilang:latest
	docker push srilanglang/srilang:latest
	docker tag srilang srilanglang/srilang:$(firstword $(subst +, ,$(shell docker run srilang --version)))
	docker push srilanglang/srilang:$(firstword $(subst +, ,$(shell docker run srilang --version)))
