...
from srilang.exceptions import CompilerPanic, SyntaxException
from srilang.typing import ClassTypes
...
class AnnotatingVisitor(python_ast.NodeTransformer):
    ...
    def generic_visit(self, node):
        """
        Annotate a node with information that simplifies srilang node generation.
        """
    ...
    def visit_ClassDef(self, node):
        """
        Annotate the Class node with it's original type from the srilang source.

        srilang uses `struct` and `contract` in place of `class`, however these
...
def annotate_python_ast(
    ...
    """
    Annotate and optimize a Python AST in preparation conversion to a srilang AST.
