...
def parse_natspec(
    srilang_module: sri_ast.Module, global_ctx: GlobalContext,
) -> Tuple[dict, dict]:
    """
    ...
    """
    userdoc, devdoc = {}, {}
    source: str = srilang_module.full_source_code

    docstring = srilang_module.get("doc_string.value")
    if docstring:
        devdoc.update(_parse_docstring(source, docstring, ("param", "return")))
        if "notice" in devdoc:
            userdoc["notice"] = devdoc.pop("notice")

    for node in [i for i in srilang_module.body if i.get("doc_string.value")]:
        docstring = node.doc_string.value
        sigs = sig_utils.mk_single_method_identifier(node, global_ctx)

        if isinstance(node.returns, sri_ast.Tuple):
