...
from srilang.exceptions import SyntaxException, VersionException
from srilang.typing import ClassTypes, ParserPosition
...
def validate_version_pragma(version_str: str, start: ParserPosition) -> None:
    ...
    """
    from srilang import __version__
...
def pre_parse(code: str) -> Tuple[ClassTypes, str]:
    """
    Re-formats a srilang source string into a python source string and performs
    ...
    * Prevents use of python semi-colon statement separator
    ...
    respective srilang class types ("contract" or "struct").

    Parameters
    ----------
    code : str
        The srilang source code to be re-formatted.
    ...
    try:
        ...
        for token in g:
            ...
            # Translate srilang-specific class keywords into python "class"
            # keyword
            if typ == NAME and string in srilang_CLASS_TYPES and start[1] == 0:
