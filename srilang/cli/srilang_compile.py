...
from srilang.parser import parser_utils
from srilang.settings import srilang_TRACEBACK_LIMIT
from srilang.signatures.interface import extract_file_interface_imports
from srilang.typing import ContractCodes, ContractPath, OutputFormats
...
interface          - srilang interface of a contract
...
def _parse_args(argv):
    ...
    parser.add_argument(
        'input_files',
        help='srilang sourcecode to compile',
        nargs='+',
    )
    parser.add_argument(
        '--version',
    ...
    if args.traceback_limit is not None:
    ...
    elif srilang_TRACEBACK_LIMIT is not None:
        sys.tracebacklimit = srilang_TRACEBACK_LIMIT
    else:
        ...
        # an error occurred in a srilang source file.
...
def get_interface_file_path(base_paths: Sequence, import_path: str) -> Path:
    ...
    for path in base_paths:
        ...
        suffix = next((i for i in ('.sri', '.json') if file_path.with_suffix(i).exists()), None)
        if suffix:
            return file_path.with_suffix(suffix)
    raise FileNotFoundError(f" Cannot locate interface '{import_path}{{.sri,.json}}'")
...
def compile_files(input_files: Iterable[str],
    ...
    compiler_data = srilang.compile_codes(
        contract_sources,
        final_formats,
    ...
    if show_version:
        compiler_data['version'] = srilang.__version__
