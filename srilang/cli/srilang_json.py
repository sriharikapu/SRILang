...
import srilang
from srilang.cli.srilang_compile import get_interface_file_path
from srilang.exceptions import JSONError
from srilang.opcodes import DEFAULT_EVM_VERSION, EVM_VERSIONS
from srilang.signatures.interface import extract_file_interface_imports
from srilang.typing import ContractCodes, ContractPath
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description='srilang programming language for EVM - JSON Compiler',
        formatter_class=argparse.RawTextHelpFormatter
    )
    parser.add_argument(
        'input_file',
        help='JSON file to compile from. If none is given, srilang will receive it from stdin.',
        nargs='?',
    )
    parser.add_argument(
        '--version',
        action='version',
        version=f'{srilang.__version__}+commit.{srilang.__commit__}',
    )
    parser.add_argument(
        '-o',
    ...
    parser.add_argument(
        '-p',
        help='Set a base import path. srilang searches here if a file is not found in the JSON.',
...
def exc_handler_to_dict(file_path: Union[str, None],
    ...
    output_json = {
        'compiler': f"srilang-{srilang.__version__}",
        'errors': [err_dict],
    }
    return output_json
...
def _standardize_path(path_str: str) -> str:
    root_path = Path('/__srilang').resolve()
...
def get_input_dict_settings(input_dict: Dict) -> Dict:
    ...
    if evm_version in ('homestead', 'tangerineWhistle', 'spuriousDragon'):
        raise JSONError("srilang does not support pre-byzantium EVM versions")
...
def get_input_dict_interfaces(input_dict: Dict) -> Dict:
    ...
    for path, value in input_dict.get('interfaces', {}).items():
        ...
        elif key.endswith(".sri"):
            if 'content' not in value:
                raise JSONError(f"Interface '{path}' must have 'content' field")
            interface = {'type': "srilang", 'code': value['content']}
        else:
            raise JSONError(f"Interface '{path}' must have suffix '.sri' or '.json'")
...
def get_interface_codes(root_path: Union[Path, None],
    ...
    for interface_name, interface_path in interface_codes.items():
        ...
        key = next((i+".sri" for i in keys if i+".sri" in contract_sources), None)
        if key:
        ...
        if root_path is None:
            raise FileNotFoundError(f"Cannot locate interface '{interface_path}{{.sri,.json}}'")
...
def compile_from_input_dict(input_dict: Dict,
    ...
    if input_dict['language'] != "srilang":
        raise JSONError(f"Invalid language '{input_dict['language']}' - Only srilang is supported.")
...
def compile_json(input_json: Union[Dict, str],
    ...
    try:
        ...
        return exc_handler(file_path, exc, "srilang")
