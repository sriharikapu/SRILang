...
from srilang import compile_lll, opcodes
from srilang.ast import ast_to_dict, parse_natspec
from srilang.compiler.phases import CompilerData
from srilang.compiler.utils import build_gas_estimates
...
def build_ast_dict(compiler_data: CompilerData) -> dict:
    ast_dict = {
        ...
        "ast": ast_to_dict(compiler_data.srilang_module),
    }
    return ast_dict
...
def build_devdoc(compiler_data: CompilerData) -> dict:
    userdoc, devdoc = parse_natspec(
        compiler_data.srilang_module, compiler_data.global_ctx
    )
    return devdoc
