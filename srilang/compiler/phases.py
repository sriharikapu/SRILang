...
from srilang import compile_lll, optimizer
from srilang.parser import parser
...
class CompilerData:
    """
    Object for fetching and storing compiler data for a srilang contract.
    ...
    srilang_module_folded : sri_ast.Module
        Folded srilang AST
    ...
    def __init__(
        self,
        source_code: str,
        contract_name: str = "srilangContract",
        ...
        interface_codes: Dict, optional
            ...
            * Formatted as as `{'interface name': {'type': "json/srilang", 'code': "interface code"}}`
            * JSON interfaces are given as lists, srilang interfaces as strings
    ...
    @property
    def srilang_module(self) -> sri_ast.Module:
        if not hasattr(self, "_srilang_module"):
            self._srilang_module = generate_ast(self.source_code, self.source_id)

        return self._srilang_module

    @property
    def srilang_module_folded(self) -> sri_ast.Module:
        if not hasattr(self, "_srilang_module_folded"):
            self._srilang_module_folded = generate_folded_ast(self.srilang_module)

        return self._srilang_module_folded

    @property
    def global_ctx(self) -> GlobalContext:
        if not hasattr(self, "_global_ctx"):
            self._global_ctx = generate_global_context(
                self.srilang_module_folded, self.interface_codes
            )
...
def generate_ast(source_code: str, source_id: int) -> sri_ast.Module:
    """
    Generate a srilang AST from source code.
    ...
    sri_ast.Module
        Top-level srilang AST node
    """
    return sri_ast.parse_to_ast(source_code, source_id)


def generate_folded_ast(srilang_module: sri_ast.Module) -> sri_ast.Module:
    """
    Perform constant folding operations on the srilang AST.
    ...
    sri_ast.Module
        Folded srilang AST
    """
    srilang_module_folded = copy.deepcopy(srilang_module)
    sri_ast.folding.fold(srilang_module_folded)

    return srilang_module_folded
