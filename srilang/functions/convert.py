...
from srilang.exceptions import InvalidLiteral, StructureException, TypeMismatch
from srilang.functions.signatures import signature
from srilang.parser.parser_utils import LLLnode, byte_array_to_num, getpos
from srilang.types import BaseType, ByteArrayType, StringType, get_type
from srilang.utils import DECIMAL_DIVISOR, MemoryPositions, SizeLimits
...
def convert(expr, context):
    ...
    if isinstance(expr.args[1], sri_ast.Name):
        ...
        isinstance(expr.args[1], (sri_ast.Subscript)) and
        isinstance(expr.args[1].value, (sri_ast.Name))
    ):
        output_type = expr.args[1].value.id
    else:
        raise StructureException("Invalid conversion type, use valid srilang type.", expr)
