...
from srilang.ast.validation import validate_call_args
...
from srilang.functions.convert import convert
from srilang.opcodes import version_check
from srilang.parser.expr import Expr
from srilang.parser.keccak256_helper import keccak256_helper
from srilang.parser.parser_utils import (
    LLLnode,
    add_variable_offset,
    get_length,
...
# srilang.parser.stmt so we can use _assert_reason).
...
class Floor:
    ...
    def evaluate(self, node):
        ...
        if not isinstance(node.args[0], sri_ast.Decimal):
            raise UnfoldableNode

        value = math.floor(node.args[0].value)
        return sri_ast.Int.from_node(node, value=value)
...
class Len:
    ...
    def evaluate(self, node):
        validate_call_args(node, 1)
        arg = node.args[0]
        if isinstance(arg, (sri_ast.Str, sri_ast.Bytes)):
            length = len(arg.value)
        elif isinstance(arg, sri_ast.Hex):
            # 2 characters represent 1 byte and we subtract 1 to ignore the leading `0x`
            length = len(arg.value) // 2 - 1
        else:
            raise UnfoldableNode

        return sri_ast.Int.from_node(node, value=length)
...
class Keccak256:
    ...
    def evaluate(self, node):
        validate_call_args(node, 1)
        if isinstance(node.args[0], sri_ast.Bytes):
            value = node.args[0].value
        elif isinstance(node.args[0], sri_ast.Str):
            value = node.args[0].value.encode()
        elif isinstance(node.args[0], sri_ast.Hex):
        ...
        return sri_ast.Hex.from_node(node, value=hash_)
...
class MethodID:
    ...
    def evaluate(self, node):
        ...
        args = node.args
        if not isinstance(args[0], sri_ast.Str):
        ...
        if isinstance(args[1], sri_ast.Name) and args[1].id == "bytes32":
            length = 32
        elif (
            isinstance(args[1], sri_ast.Subscript) and
        ...
        if length == 32:
            return sri_ast.Hex.from_node(node, value=f"0x{value.hex()}")
        elif length == 4:
            return sri_ast.Bytes.from_node(node, value=value)
        else:
            raise CompilerPanic
...
class AsWeiValue:
    ...
    def evaluate(self, node):
        validate_call_args(node, 2)
        if not isinstance(node.args[1], sri_ast.Str):
            raise ArgumentException(
                "Wei denomination must be given as a literal string", node.args[1]
            )
        if not isinstance(node.args[0], (sri_ast.Decimal, sri_ast.Int)):
            raise UnfoldableNode
        value = node.args[0].value
        ...
        return sri_ast.Int.from_node(node, value=int(value * denom))
...
class RawLog:
    ...
    @validate_inputs
    def build_LLL(self, expr, args, kwargs, context):
        if not isinstance(args[0], sri_ast.List) or len(args[0].elts) > 4:
...
class BitwiseAnd:
    ...
    def evaluate(self, node):
        ...
        for arg in node.args:
            if not isinstance(arg, sri_ast.Num):
                raise UnfoldableNode
...
class BitwiseNot:
    ...
    def evaluate(self, node):
        validate_call_args(node, 1)
        if not isinstance(node.args[0], sri_ast.Num):
            raise UnfoldableNode
...
class Shift:
    ...
    def evaluate(self, node):
        validate_call_args(node, 2)
        if [i for i in node.args if not isinstance(i, sri_ast.Num)]:
            raise UnfoldableNode
...
def get_create_forwarder_to_bytecode():
    from srilang.compile_lll import (
        assembly_to_evm,
        num_to_bytearray
    )
    code_a = [
...
class Sqrt:
    ...
    @validate_inputs
    def build_LLL(self, expr, args, kwargs, context):
        from srilang.functions.utils import (
