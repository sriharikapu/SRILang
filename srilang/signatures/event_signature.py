...
from srilang.types import ByteArrayType, canonicalize_type, get_size_of_type
    ...
    def from_declaration(cls, code, global_ctx):
        ...
        if code.annotation.args:
            ...
            for i in range(len(keys)):
                ...
                if not isinstance(typ, (sri_ast.Name, sri_ast.Call, sri_ast.Subscript)):
                    raise EventDeclarationException('Invalid event argument type.', typ)
                if isinstance(typ, sri_ast.Call) and not isinstance(typ.func, sri_ast.Name):
                    raise EventDeclarationException('Invalid event argument type', typ)
                arg = keys[i].id
                arg_item = keys[i]
                is_indexed = False
                ...
                if isinstance(typ, sri_ast.Subscript) and getattr(typ.value, 'id', None) == 'bytes' and typ.slice.value.n > 32 and is_indexed:  # noqa: E501
