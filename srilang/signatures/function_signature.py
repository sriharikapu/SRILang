    ...
    def from_definition(cls,
        ...
        for dec in code.decorator_list:
            if isinstance(dec, sri_ast.Name) and dec.id == "constant":
                ...
                public = True
            elif isinstance(dec, sri_ast.Call) and dec.func.id == "nonreentrant":
                if nonreentrant_key:
                    raise StructureException(
                        "Only one @nonreentrant decorator allowed per function",
                        dec
                    )
                if dec.args and len(dec.args) == 1 and isinstance(dec.args[0], sri_ast.Str) and dec.args[0].s:  # noqa: E501
        ...
        if not code.returns:
            ...
            code.returns,
            (sri_ast.Name, sri_ast.Compare, sri_ast.Subscript, sri_ast.Call, sri_ast.Tuple)
        ):
...
def validate_default_values(node):
    if isinstance(node, sri_ast.Name) and node.id in parser.expr.BUILTIN_CONSTANTS:
        return
    if isinstance(node, sri_ast.Attribute) and node.value.id in parser.expr.ENVIRONMENT_VARIABLES:
        return
    if not isinstance(node, (sri_ast.Constant, sri_ast.List)):
        raise FunctionDeclarationException(
            "Default value must be a literal, built-in constant, or environment variable.",
            node
        )
    if isinstance(node, sri_ast.List):
        for n in node.elts:
