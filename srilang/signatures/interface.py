...
from srilang.types.types import ByteArrayLike, TupleLike
from srilang.typing import InterfaceImports, SourceCode


# Populate built-in interfaces.
def get_builtin_interfaces():
    interface_names = [x.name for x in pkgutil.iter_modules(srilang.interfaces.__path__)]
    return {
        name: extract_sigs({
            'type': 'srilang',
            'code': importlib.import_module(
                f'srilang.interfaces.{name}',
            ).interface_code,
        })
...
# srilang defines a maximum length for bytes and string types, but Solidity does not.
...
# Ths approach solves the issue because srilang allows for an implicit casting
...
def mk_full_signature_from_json(abi):
    ...
    for func in funcs:
        ...
        decorator_list = [sri_ast.Name(id='public')]
        if func['constant']:
            decorator_list.append(sri_ast.Name(id='constant'))
        if func['payable']:
            decorator_list.append(sri_ast.Name(id='payable'))

        sig = FunctionSignature.from_definition(
            code=sri_ast.FunctionDef(
                name=func['name'],
                args=sri_ast.arguments(args=args),
