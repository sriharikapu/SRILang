...
from srilang.ast import folding
...
def test_replace_literal_ops():
    test_ast = sri_ast.parse_to_ast("[not True, True and False, True or False]")
...
def test_replace_constant(source):
    ...
    assert not sri_ast.compare_nodes(unmodified_ast, folded_ast)
