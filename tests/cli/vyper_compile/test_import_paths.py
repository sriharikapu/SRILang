...
from srilang.cli.srilang_compile import compile_files, get_interface_file_path
...
def test_import_same_folder(import_stmt, tmp_path):
    ...
    foo_path = tmp_path.joinpath('contracts/foo.sri')
...
def test_import_subfolder(import_stmt, tmp_path):
    ...
    with tmp_path.joinpath('contracts/other/Bar.sri').open('w') as fp:
        fp.write(BAR_CODE)
...
def test_import_other_folder(import_stmt, tmp_path):
    ...
    with tmp_path.joinpath('interfaces/Bar.sri').open('w') as fp:
        fp.write(BAR_CODE)
...
def test_import_parent_folder(tmp_path, assert_compile_failed):
    ...
    foo_path = tmp_path.joinpath('contracts/baz/foo.sri')
    with foo_path.open('w') as fp:
        fp.write(FOO_CODE.format("from ... import Bar"))

    with tmp_path.joinpath('Bar.sri').open('w') as fp:
        fp.write(BAR_CODE)
...
def be_known() -> uint256:
    ...
    meta_path = tmp_path.joinpath('contracts/Meta.sri')
...
def bar(_foo: address, _bar: address) -> uint256:
    ...
    with tmp_path.joinpath('Foo.sri').open('w') as fp:
        fp.write(FOO_CODE.format(import_stmt_foo))

    with tmp_path.joinpath('Bar.sri').open('w') as fp:
        fp.write(BAR_CODE)

    baz_path = tmp_path.joinpath('Baz.sri')
...
def test_local_namespace(tmp_path):
    ...
    for i, code in enumerate(codes):
        path = tmp_path.joinpath(f'code{i}.sri')
        with path.open('w') as fp:
            fp.write(code)
        compile_paths.append(path)

    for file_name in ('foo.sri', 'bar.sri'):
...
def test_get_interface_file_path(tmp_path):
    for file_name in ('foo.sri', 'foo.json', 'bar.sri', 'baz.json', 'potato'):
        with tmp_path.joinpath(file_name).open('w') as fp:
            fp.write("")
    ...
    assert get_interface_file_path(base_paths, 'foo') == tmp_path.joinpath('foo.sri')
    assert get_interface_file_path(base_paths, 'bar') == tmp_path.joinpath('bar.sri')
...
def test_compile_outside_root_path(tmp_path):
    foo_path = tmp_path.joinpath('foo.sri')
    with foo_path.open('w') as fp:
        fp.write(FOO_CODE.format('import bar as Bar'))

    bar_path = tmp_path.joinpath('bar.sri')
