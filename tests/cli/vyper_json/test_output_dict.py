...
import srilang
from srilang.cli.srilang_json import format_to_output_dict
from srilang.compiler import OUTPUT_FORMATS, compile_codes
...
def test_keys():
    ...
    assert output_json['compiler'] == f"srilang-{srilang.__version__}"
    data = compiler_data['foo.sri']
    assert output_json['sources']['foo.sri'] == {'id': 0, 'ast': data['ast_dict']['ast']}
    assert output_json['contracts']['foo.sri']['foo'] == {
        'abi': data['abi'],
        'devdoc': data['devdoc'],
