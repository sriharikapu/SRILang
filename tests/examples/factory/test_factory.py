...
def create_token(get_contract):
    with open('examples/tokens/ERC20.sri') as f:
        code = f.read()

    def create_token():
        return get_contract(code, *['srilangCoin', 'FANG', 0, 0])
...
def create_exchange(w3, get_contract):
    with open('examples/factory/Exchange.sri') as f:
        code = f.read()
...
def factory(get_contract):
    ...
    exchange_interface = srilang.compile_code(code, output_formats=['bytecode_runtime'])
    exchange_deployed_bytecode = exchange_interface['bytecode_runtime']

    with open('examples/factory/Factory.sri') as f:
        code = f.read()
