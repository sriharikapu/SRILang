...
from srilang.compiler import compile_code, compile_codes
from srilang.exceptions import StructureException
from srilang.interfaces import ERC20, ERC721
...
contract One:
    ...
    out = compile_codes({'one.sri': code}, ['external_interface'])['one.sri']
...
def test_builtin_interfaces_parse():
    assert len(extract_sigs({'type': 'srilang', 'code': ERC20.interface_code})) == 8
    assert len(extract_sigs({'type': 'srilang', 'code': ERC721.interface_code})) == 13
...
def foo() -> uint256:
    ...
    base = extract_sigs({'type': 'srilang', 'code': interface_code.format("")})

    for stmt in ("import x as x", "from x import y"):
        sigs = extract_sigs({'type': 'srilang', 'code': interface_code.format(stmt)})
...
def test():
    ...
    test_c = get_contract(code, *[erc20.address], interface_codes={
        'TokenCode': {'type': 'srilang', 'code': token_code}
    })
